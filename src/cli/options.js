...
const OPTIONS = {
  ...
  'ai-describe': {
    group: 'AI options:',
    description: 'Generate a BLIP caption for every image (local, no network)',
    type: 'boolean',
    'default': false
  },
  'ai-ocr': {
    group: 'AI options:',
    description: 'Run Tesseract OCR on every image and index the text for search',
    type: 'boolean',
    'default': false
  },
  'ai-python': {
    group: 'AI options:',
    description: 'Python executable used to run scripts/ai_describe.py',
    type: 'string',
    'default': 'python3'
  },
  'ai-blip-model': {
    group: 'AI options:',
    description: 'HuggingFace model id for BLIP captioning',
    type: 'string',
    'default': 'Salesforce/blip-image-captioning-base'
  },
  'ai-ocr-engine': {
    group: 'AI options:',
    description: 'OCR backend. easyocr is GPU-capable (CUDA when available, CPU fallback). tesseract is CPU-only with a lighter Python install.',
    choices: ['easyocr', 'tesseract'],
    'default': 'easyocr'
  },
  'ai-embed': {
    group: 'AI options:',
    description: 'Generate sentence-transformer embeddings of (caption+OCR) and ship them with the search index. Adds semantic search ("dog" matches "puppy") on top of the BM25 keyword search, blended via Reciprocal Rank Fusion.',
    type: 'boolean',
    'default': false
  },
  'ai-embed-model': {
    group: 'AI options:',
    description: 'Sentence-transformer model id. Must have a Xenova ONNX export available for client-mode browser embedding.',
    type: 'string',
  ...
  'search-mode': {
    ...
    description: 'How the search page queries its index. "client" ships a prebuilt MiniSearch index to the browser (fully static). "server" builds a Whoosh index and needs a running Python server to answer queries.',
    choices: ['client', 'server'],
    'default': 'client'
  },
  'search-python': {
    group: 'Search options:',
    description: 'Python executable used to build the Whoosh index (server mode only)',
    type: 'string',
    'default': 'python3'
  },
  'search-build-procs': {
    group: 'Search options:',
    description: 'Whoosh: number of parallel indexing processes (server mode only)',
    type: 'number',
    'default': 1
  },
  'search-build-multisegment': {
    group: 'Search options:',
    description: 'Whoosh: skip segment merge on commit. Faster builds for huge galleries; slightly slower searches until you optimize',
    type: 'boolean',
    'default': false
  },
