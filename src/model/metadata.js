...
class Metadata {
  constructor (exiftool, picasa, opts) {
    ...
    // AI-generated search fields, populated later by the AI step.
    // Kept separate (not concatenated) so the UI can target each independently.
    this.aiCaption = ''
    this.aiOcr = ''
    // Sentence-transformer embedding of (aiCaption + " " + aiOcr) for
    // hybrid semantic search. Float32Array (dim depends on model) or null.
    this.aiEmbedding = null
