...
class Theme {
  ...
  prepare (done) {
    ...
    // Optional: compile a search template if the theme provides one. Used
    // by website.js to render the standalone search page through the same
    // handlebars + partials + helpers pipeline as the gallery albums.
    const searchHbs = path.join(this.dir, 'search.hbs')
    if (fs.existsSync(searchHbs)) {
      this.searchTemplate = compileTemplate(searchHbs)
    } else {
  ...
  // Renders the search page through the theme pipeline. Returns false (so
  // callers can fall back) if this theme doesn't ship a search.hbs.
  renderSearch (targetPath, data, next) {
    if (!this.searchTemplate) return false
    const fullPath = path.join(this.dest, targetPath)
    debug(`Theme rendering search page ${targetPath}`)
    const contents = this.searchTemplate(data)
    fs.mkdirSync(path.dirname(fullPath), { recursive: true })
    fs.writeFile(fullPath, contents, next)
    return true
  }
